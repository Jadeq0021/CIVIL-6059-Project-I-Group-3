...
namespace CodeAgent
{
    internal class Module
    {
        ...
        public UV insert { get; set; }
        public Module(Document document, UV point_1, UV point_2, UV point_3, UV point_4, Level level, string name, string wallThickness_string)
        {
            ...
            if (wall_1 == null)
            {
                wall_1 = Utils.CreateWall(this.document, point_1, point_2, this.level, name, wallThickness_string);
            }
            Wall wall_2 = Utils.GetWallByTwoPoints(document, point_2, point_3);
            if (wall_2 == null)
            {
                wall_2 = Utils.CreateWall(this.document, point_2, point_3, this.level, name, wallThickness_string);
            }
            Wall wall_3 = Utils.GetWallByTwoPoints(document, point_3, point_4);
            if (wall_3 == null)
            {
                wall_3 = Utils.CreateWall(this.document, point_3, point_4, this.level, name, wallThickness_string);
            }
            Wall wall_4 = Utils.GetWallByTwoPoints(document, point_4, point_1);
            if (wall_4 == null)
            {
                wall_4 = Utils.CreateWall(this.document, point_4, point_1, this.level, name, wallThickness_string);
            }
            Floor floor = Utils.CreateFloor(document, point_1, point_2, point_3, point_4, level, name);
        }

        public Module(Document document, List<UV> points, Level level, string name, List<double> wallThicknesses,UV insert)
        {
            this.document = document;
            this.points = points;
            this.level = level;
            ...
            this.insert = insert;
            //遍历points 中的每个点 加上insert
            for (int i = 0; i < this.points.Count;i++)
            {
                this.points[i] += insert;
            }

            CreateWalls(wallThicknesses);
            CreateFloor();
        }
        private void CreateWalls(List<double> wallThicknesses)
        {
            for (int i = 0; i < points.Count; i++)
            {
                int nextIndex = (i + 1) % points.Count;
                UV point1 = points[i];
                UV point2 = points[nextIndex];

                double wallThickness = wallThicknesses[i % wallThicknesses.Count];
                Wall wall = Utils.CreateWall(document, point1, point2, level, name, wallThickness);
                if (wall != null)
                {
                    Walls.Add(wall);
                }
        ...
        {
            Floor = Utils.CreateFloor(document, points, level, name);
        }
